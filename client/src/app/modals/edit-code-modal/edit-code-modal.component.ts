...
export class EditCodeModalComponent implements OnInit, AfterViewInit, OnDestroy {
  @Input() module!: Module;
  ...
  submit() {
    const payload = {
      ...
      assessments: this.module.assessments || []
    };

    this.http.put(`${this.baseUrl}modules/${this.module.id}`, payload).subscribe({
      next: () => { this.toastr.success('Module code updated successfully'); this.justSaved = true; this.originalHide(); },
      error: err => { this.toastr.error('Failed to update module code'); console.error(err); }
    });
  }
